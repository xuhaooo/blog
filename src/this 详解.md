...
于是，this 出现了，它出现的目的就是：在函数内，指代函数当前运行的环境，或者说上下文
...
方法一：依然在调用时把第一个参数 self 当做 user，这样形参就永远比实参多一个 self
...
```
  ...
  按理说打印出来的应该是 undefined，但是浏览器中有这样的规则：

  > 如果你传的 context 是 undefined / null，那么 window 对象就是默认的 context（严格模式下 undefined 是默认的 context）

  如果你不希望这里的 this 不是 window，可以这样写`fn.call(obj)` 之后里面的 this 就是 obj 了
...
- 回到文章开头的例子，这才是 JS 设计者提供的无糖版调用函数的方法
...
- 注意，它创建了一个新函数，是原函数的拷贝；改变新函数的的 this 和参数；return 这个新函数，你需要手动调用
